...
def plot_det_curve(fpr, fnr, eer, label=None):
    ...
    plt.scatter(eer_fpr, eer_fnr, color="black", label=f"EER = {eer:.5f}", s=20)
...
def plot_roc_curve(far, tpr, eer, label=None):
    roc_auc = auc(far, tpr)
    plt.figure(figsize=(5, 5))
    plt.plot(far, tpr, label=f"ROC Curve (AUC = {roc_auc:.5f})")
    eer_index = np.nanargmin(np.absolute(far - (1 - tpr)))
    plt.scatter(far[eer_index], tpr[eer_index], color="black", label=f"EER = {eer:.5f}", s=20)
    plt.plot([0, 1], [0, 1], color="black", lw=1, linestyle="--")
